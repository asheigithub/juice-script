package  
{
	...
	public final class ushort 
	{
		...
		/**
		 * 返回指定 ushort 对象的原始 ushort 类型值。
		 * @return	此 ushort 对象的原始 ushort 类型值。
		 * @langversion	3.0
		 */
		public native function valueOf () : ushort;
		
		/**
		 * 返回 ushort 对象的字符串表示形式。
		 * @param	radix	指定要用于数字到字符串的转换的基数（从 2 到 36）。如果未指定 radix 参数，则默认值为 10。
		 * @return	ushort 对象的字符串表达式。
		 * @langversion	3.0
		 */
		public native function toString (radix:int = 10) : String;
		
		/**
		 * 创建新的 ushort 对象。可以创建一个 ushort 类型的变量并赋予其文本值。
		 * @param	value	要创建的 ushort 对象的数值，或者要转换为数字的值。如果未提供 value，则默认值为 0。
		 * @langversion	3.0
		 */
		public native function ushort (value:*= 0);
