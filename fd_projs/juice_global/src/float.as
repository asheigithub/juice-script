package 
{
	...
	public final class float 
	{
		 ...
		 */
		public native function valueOf () : float;
		
		/**
		 * 返回 float 对象的字符串表示形式。
		 * @param	radix	指定要用于数字到字符串的转换的基数（从 2 到 36）。如果未指定 radix 参数，则默认值为 10。
		 * @return	float 对象的字符串表达式。
		 */
		public native function toString (radix:int = 10) : String;
		
		/**
		 * 创建新的 float 对象。可以创建一个 float 类型的变量并赋予其文本值。
		 * @param	value	要创建的 float 对象的数值，或者要转换为数字的值。如果未提供 value，则默认值为 0。
		 */
		public native function float (value:* = 0);
