	...
	internal class Program
	{
		...
		private unsafe static void OnLoad()
		{
			...
			//加载box2d-lite脚本
			{
				...
				NaNBoxing world_value = ((RtScriptClass)Player.HeapShortCut[doc.__global_index__]).ReadSlot(index);
				world = (RtInstance)Player.HeapShortCut[world_value.HeapPtr];
		...
		private unsafe static void OnRender(double obj)
		{
			...
			for (int i = 0; i < body_count ; i++)
			{
				...
				RtInstance body = (RtInstance)Player.HeapShortCut[body_v.HeapPtr];
				...
				RtInstance pos = (RtInstance)Player.HeapShortCut[pos_v.HeapPtr];

				var width_v = body.ReadSlot((ushort)body_width, Player);
				RtInstance width = (RtInstance)Player.HeapShortCut[width_v.HeapPtr];
			...
			for (int i = 0; i < Player.GetVectorLen(joints_v); i++)
			{
				...
				RtInstance joint = (RtInstance)Player.HeapShortCut[joint_v.HeapPtr];

				var body1_v = joint.ReadSlot((ushort)joint_body1, Player);
				RtInstance body1 = (RtInstance)Player.HeapShortCut[body1_v.HeapPtr];
				...
				RtInstance pos1 = (RtInstance)Player.HeapShortCut[pos1_v.HeapPtr];

				var body2_v = joint.ReadSlot((ushort)joint_body2, Player);
				RtInstance body2 = (RtInstance)Player.HeapShortCut[body2_v.HeapPtr];
				...
				RtInstance pos2 = (RtInstance)Player.HeapShortCut[pos2_v.HeapPtr];

				var localAnchor1_v = joint.ReadSlot((ushort) joint_localAnchor1, Player);
				RtInstance localAnchor1 = (RtInstance)Player.HeapShortCut[localAnchor1_v.HeapPtr];

				var localAnchor2_v = joint.ReadSlot((ushort)joint_localAnchor2, Player);
				RtInstance localAnchor2 = (RtInstance)Player.HeapShortCut[localAnchor2_v.HeapPtr];
			...
			for (int i = 0; i < alen; i++)
			{
				...
				RtInstance arbiter = (RtInstance)Player.HeapShortCut[arbiter_v.HeapPtr];
				...
				for (int j = 0; j < num; j++)
				{
					...
					RtInstance contact = (RtInstance)Player.HeapShortCut[contact_v.HeapPtr];
