...
namespace MyMD5
{
    ...
    public struct MD5Result : IEquatable<MD5Result>
    {
        ...
        [FieldOffset(8)]
        public readonly ulong P2;

        
        public MD5Result(ulong P1,ulong P2)
        { 
        ...
        public override string ToString()
        {
            unsafe
            {
                ...
                return Uri.EscapeDataString( Convert.ToBase64String(new ReadOnlySpan<byte>(&v1,sizeof(ulong))) + Convert.ToBase64String(new ReadOnlySpan<byte>(&v2,sizeof(ulong) ) ));
            }
        }

        public bool Equals(MD5Result other)
        {
    ...
    public class MyMD5
    {
        ...
        public MD5Result Hash(string data)
        {
            Full_m_byteInput(data);
            CalculateMD5Value();

            MD5Result result = new MD5Result(
            (ulong)ReverseByte(A) << 32 | ReverseByte(B),
            (ulong)ReverseByte(C) << 32 | ReverseByte(D));
