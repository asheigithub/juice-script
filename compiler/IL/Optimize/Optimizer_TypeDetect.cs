					...
					case INS_Code.ld_method:
						{
							...
							{
								var v = result.FirstOrDefault(r => r.Key.GetDef().Contains(ld_Method.instance)).Value;
								...
								switch (v[0].DefType)
								{
									...
									case InstructionDefType.primitive:
										//throw new NotImplementedException();
										{
											if (v[0].Obj is ASInstance)
											{
												result.Add(instruction, new List<InstructionDef>()
												{
													new InstructionDef(InstructionDefType.method,
													((ASContainer)v[0].Obj)._vtable.Items[ (int)ld_Method.const_index ] )
												});
												flag = true;
											}
											else if (v[0].Obj is NaNBoxing)
											{
												NaNBoxing val = (NaNBoxing)v[0].Obj;
