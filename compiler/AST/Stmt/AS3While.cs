...
{
    public sealed class AS3While : IAS3SyntaxNode
    {
        private Token token;
        public AS3While(Token token)
        {
            this.token = token;
        }
        ...
        public void Write(int v, StringBuilder out_sb)
        {
            ...
            out_sb.AppendLine("".PadLeft(v, '\t') + "while(");
            ...
            out_sb.AppendLine("".PadLeft(v+1, '\t') + ((AS3Expression)Condition[Condition.Count - 1]).Value);
