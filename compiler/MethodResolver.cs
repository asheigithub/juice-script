										...
										for (int i = 1; i < script.scriptMethods.Count; i++)
										{
											...
											for (int j = 0; j < h_count; j++)
											{
												...
												if (kind == ASMethodBody.PoolHeapPtrKind.String)
												{
													...
													method.Body.heapConstants.pool_values[j] = context.player_for_compiler.HeapShortCut[heap_ptr];
												...
												if (record != null)
												{
													...
													else if (v.StartsWith("str:"))
													{
														...
														if (!found)
														{
															...
															hobjs.Add(context.player_for_compiler.HeapShortCut[heapptr]);
					...
					foreach (var container in script.allContainers.Where(c => c != null).OrderBy(c => (int)c._link_codescope.Kind))
					{
							...
							))
						{
							...
							try
							{
								...
								{
									var obj = computeplayer.HeapShortCut[v.HeapPtr];
						...
						for (int a = 0; a < method.Parameters.Count; a++)
						{
							...
							if (para.IsOptional)
							{
								...
								try
								{
									...
									{
										var obj = computeplayer.HeapShortCut[value.HeapPtr];
