...
{
    [TestClass]
    public sealed class FilterTest : CodeTestBase
    {
...
function runTest():void {
    ...
    var r1:Vector.<int> = v1.filter(function(item:int):Boolean {
        return item > 2;
    });
    results.push((r1.length == 3) ? 1 : 0);

    var v2:Vector.<int> = new <int>[1, 2, 3];
    var r2:Vector.<int> = v2.filter(function(item:int):Boolean {
        return item > 0;
    });
    results.push((r2.length == 3) ? 1 : 0);

    var v3:Vector.<int> = new <int>[1, 2, 3];
    var r3:Vector.<int> = v3.filter(function(item:int):Boolean {
        return item > 10;
    });
    results.push((r3.length == 0) ? 1 : 0);

    var v4:Vector.<int> = new <int>[];
    var r4:Vector.<int> = v4.filter(function(item:int):Boolean {
        return true;
    });
    results.push((r4.length == 0) ? 1 : 0);

    var v5:Vector.<int> = new <int>[10, 20, 30, 40];
    var myObj:Object = { threshold: 25 };
    var r5:Vector.<int> = v5.filter(function(item:int):Boolean {
        return item > this.threshold;
    }, myObj);
    results.push((r5.length == 2) ? 1 : 0);
    ...
    var r6:Vector.<Point> = v6.filter(function(item:Point):Boolean {
        return item.x > 0;
    });
    results.push((r6.length == 2) ? 1 : 0);
    ...
    var r7:Vector.<Point> = v7.filter(function(item:Point):Boolean {
        return item.x > 0 && item.y > 0;
    });
    results.push((r7.length == 3) ? 1 : 0);

    var v8:Vector.<Point> = new <Point>[];
    var r8:Vector.<Point> = v8.filter(function(item:Point):Boolean {
        return true;
    });
