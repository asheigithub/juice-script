...
Dictionary.prototype[3] = ""Dictionary"";
           ...
           player.ForceGC();
            {
				...
				Assert.AreEqual("undefined\r\nDictionary\r\n[class Dictionary]\r\nProperty http://adobe.com/AS3/2006/builtin::LL not found on flash.utils.Dictionary and there is no default value.\r\n", print.GetOutput());

				NaNBoxing a = rtPayload.ReadSlot(0);
				Assert.AreEqual( NaNBoxing.BoxType.HeapPtr, a.ValueType );
				Assert.AreEqual( RtHeapTypeKind.INSTANCE , player.Context.GC.Heap[a.HeapPtr].TypeKind );
