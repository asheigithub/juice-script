...
"
		...
		protected override void TestIsPass(Player player, PlayerException ex)
		{
			...
			NaNBoxing t = ((RtScriptClass)globalInstance.facility).ReadSlot(8); //原AIR运行时没有从class中取到值，但是这不合理。现在的结果是符合逻辑的
			Assert.AreEqual(NaNBoxing.BoxType.Int, t.ValueType);
			Assert.AreEqual(int.MinValue, t.IntValue);

			NaNBoxing w = ((RtScriptClass)globalInstance.facility).ReadSlot(9);//原AIR运行时没有从class中取到值，但是这不合理。现在的结果是符合逻辑的
