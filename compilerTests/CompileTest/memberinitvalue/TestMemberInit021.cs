...
"
		...
		protected override void TestIsPass(Player player, PlayerException ex)
		{
			...
			NaNBoxing t = ((RtScriptClass)globalInstance).ReadSlot(8); //此处原AIR运行结果是undefined.但是从代码分析这里明显常量已经提取，从道理上看应该是数值。
			Assert.AreEqual(NaNBoxing.BoxType.Int, t.ValueType);
			Assert.AreEqual(int.MinValue, t.IntValue);

			NaNBoxing w = ((RtScriptClass)globalInstance).ReadSlot(9); //此处原AIR运行结果是undefined.但是从代码分析这里明显常量已经提取，从道理上看应该是数值。
