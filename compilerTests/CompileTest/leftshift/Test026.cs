...
{
	[TestClass]
	public sealed class Test026 : CodeTestBase
	{
...
// CHECK#1
if ((b << 0) !== 1) {
  throw new Test262Error('#1: (new Boolean(true) << 0) === 1. Actual: ' + ((b << 0)));
}

b=false;
// CHECK#2
if ((b << 0) !== 0) {
  throw new Test262Error('#2: (false << 0) === 0. Actual: ' + ((b << 0)));
}
...
// CHECK#1
if ((b << 0) !== 1) {
  throw new Test262Error('#1: (new String(1) << 0) === 1. Actual: ' + ((b << 0)));
}
