			...
			internal unsafe void RunTimerTasks(Context context, long nowticks, Action<PlayerException> onErrorRaised, ref ReceiveError task_fault)
			{
				...
				for (int i = 0; i < _count; i++)
				{
					...
					if (task.TimeToRun < nowticks)
					{
						...
						int len; RtPayloadArray argArray = (RtPayloadArray)context.GC.Heap[task.argumentsPtr].facility;
						...
						context.player.RunMethod(callmethod, ((RtPayloadClosure)closureinstance.facility).This,
