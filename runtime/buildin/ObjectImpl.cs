...
namespace juicescript.runtime.buildin
{
	internal class ObjectImpl
	{
			...
			int stackStPos, ref ReceiveError error, int returnSlotIndex)
		{
			...
			{
				NaNBoxing sName = ((RtMethodScope)context.HeapShortCut[scope_ptr]).ReadSlot(0);
				if (sName.ValueType == NaNBoxing.BoxType.HeapPtr)
				{
					RtHeapBase n = context.HeapShortCut[sName.HeapPtr];
#if DEBUG
					...
					switch (_this.Kind)
					{
						case RtHeapTypeKind.CLASS:
							{
								context.StackSlots[returnSlotIndex].SetBoolean(find_class_prop(_this, (ASClass)((RtScriptClass)_this).Meta, name));
							}
							break;
						...
						case RtHeapTypeKind.ARRAY:
							{
								...
								if (uint.TryParse(name, out isindex))
								{
									...
									NaNBoxing result = ((RtArray)_this).ReadSlot(isindex, out isoutofindex_or_ishole);
		...
		public static void Object_isPrototypeOf(Context context,
			...
			int stackStPos, ref ReceiveError error, int returnSlotIndex)
		{
			var theClass = ((RtMethodScope)context.HeapShortCut[scope_ptr]).ReadSlot(0);
...
---*/
			...
			if (thisPtr.ValueType == NaNBoxing.BoxType.HeapPtr)
			{
				...
				else
				{
					...
					int i = 0;
					var V = context.HeapShortCut[theClass.HeapPtr];
			...
			int stackStPos, ref ReceiveError error, int reseveSlot)
		{
			...
			if (obj_ins.Kind == RtHeapTypeKind.ARRAY)
			{
				...
				if (_index.IntValue < 0)
				{
					...
					if(((RtArray)obj_ins).TryReadIterItem(  _count.IntValue , out k,out next_index,out v, context) )
					{
			...
			if(property_ptr > 0)
			{
				var dynamic = context.HeapShortCut[property_ptr];
#if DEBUG
...
#endif
				RtDynamic prop = (RtDynamic)dynamic;
			...
			lbl_skip_not_enumerable:
				if (_index.IntValue < _count.IntValue && _index.IntValue < prop.Slots.Count)
				{
					...
					{
						var shape = context.HeapShortCut[shape_ptr];
#if DEBUG
...
#endif
						shape_ptr = ((RtShape)shape).PTR_PARENT;
						current++;
					}

					var shapepayload = ((RtShape)context.HeapShortCut[shape_ptr]);
