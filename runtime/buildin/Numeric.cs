...
namespace juicescript.runtime.buildin
{
	internal class Numeric
	{
			...
			int stackStPos, ref ReceiveError error, int returnSlotIndex)
		{
			...
			if (x == 0)
			{
				context.StackSlots[returnSlotIndex].SetHeapPtr(context.player.ZERO_STR, (byte)RtHeapTypeKind.STRING, (byte)HeapKindFlag.NONE);
				return;
			}

			if (double.IsPositiveInfinity(x) || x >= double.MaxValue)
			{
				context.StackSlots[returnSlotIndex].SetHeapPtr(context.player.POSITIVEINF_STR, (byte)RtHeapTypeKind.STRING, (byte)HeapKindFlag.NONE);
				return;
			}

			if (double.IsNegativeInfinity(x) || x <= double.MinValue)
			{
				context.StackSlots[returnSlotIndex].SetHeapPtr(context.player.NEGATIVEINF_STR, (byte)RtHeapTypeKind.STRING, (byte)HeapKindFlag.NONE);
				return;
			}
			...
			int stackStPos, ref ReceiveError error, int returnSlotIndex)
		{
			...
			{
				context.StackSlots[returnSlotIndex].SetHeapPtr(double.IsNegativeInfinity(x) ? context.player.NEGATIVEINF_STR : context.player.POSITIVEINF_STR, (byte)RtHeapTypeKind.STRING, (byte)HeapKindFlag.NONE);
				return;
			}
