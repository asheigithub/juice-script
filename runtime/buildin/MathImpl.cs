...
namespace juicescript.runtime.buildin
{
	internal class MathImpl
	{
		...
		{
			NaNBoxing num = ((RtMethodScope)context.HeapShortCut[scope_ptr]).ReadSlot(0);
#if DEBUG
		...
		public static void Math_abs(Context context,
			...
			int stackStPos, ref ReceiveError error, int returnSlotIndex)
		{
			NaNBoxing val = ((RtMethodScope)context.HeapShortCut[scope_ptr]).ReadSlot(0);
		...
		public static void Math_atan2(Context context,
			...
			int stackStPos, ref ReceiveError error, int returnSlotIndex)
		{
			NaNBoxing y = ((RtMethodScope)context.HeapShortCut[scope_ptr]).ReadSlot(0);
			NaNBoxing x = ((RtMethodScope)context.HeapShortCut[scope_ptr]).ReadSlot(1);
			...
			int stackStPos, ref ReceiveError error, int returnSlotIndex)
		{
			...
			{
				var slot = scope.ReadSlot(slotIdx);
				...
				else
				{
					...
					var restArray = (RtArray)context.HeapShortCut[slot.HeapPtr];
		...
		public static void Math_pow(Context context,
			...
			int stackStPos, ref ReceiveError error, int returnSlotIndex)
		{
			NaNBoxing bas = ((RtMethodScope)context.HeapShortCut[scope_ptr]).ReadSlot(0);
			NaNBoxing pow = ((RtMethodScope)context.HeapShortCut[scope_ptr]).ReadSlot(1);
