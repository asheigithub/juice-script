...
namespace juicescript.runtime
{
	public partial class Player
	{
					...
					case RtHeapTypeKind.INSTANCE:
						{
							...
							if (((ASInstance)instance.Type).Flags.HasFlag(ClassFlags.Struct))
							{
								...
								value.SetHeapPtr(ptr, (byte)RtHeapTypeKind.INSTANCE);
							}
							else if (value.HeapPtr < Context.CacheInstancePtr + Context.STACK_LENGTH)//((RtPayloadInstance)instance).isCache)
							{
								...
								{
									value.SetHeapPtr(src_ptr, (byte)RtHeapTypeKind.INSTANCE);
								}
							}
						}
						break;
					...
					case RtHeapTypeKind.CLOSURE:
						{
							...
							if (value.HeapPtr < Context.M_ClosurePtr + Context.STACK_LENGTH)
							{
								...
								if (src_ptr < Context.M_ClosurePtr + Context.STACK_LENGTH)
								{
									...
									{
										value.SetHeapPtr(cache.cloneing_ptr , (byte)RtHeapTypeKind.CLOSURE);
										break;
									}
									...
									value.SetHeapPtr(ptr, (byte)RtHeapTypeKind.CLOSURE);


								}
								else
								{
									value.SetHeapPtr(src_ptr, (byte)RtHeapTypeKind.CLOSURE);
								}
							}
					...
					case RtHeapTypeKind.MethodScope:
						{
							if (value.HeapPtr < Context.M_MethodScopePtr + Context.MAX_BACKTRACE)
							{
								...
								if (!((ASMethodBody)instance.Type).Method.Flags.HasFlag(MethodFlags.NeedActivation))
								{
									...
									{
										value.SetHeapPtr(0 , (byte)RtHeapTypeKind.MethodScope );
									}
									else
									{
								...
								else
								{
									...
									{
										value.SetHeapPtr(cacheMscope.cloneout_ptr, (byte)RtHeapTypeKind.MethodScope);
										break;
									}
									...
									value.SetHeapPtr(ptr, (byte)RtHeapTypeKind.MethodScope);
								}
							}
						}
						break;
					case RtHeapTypeKind.ARRAY:
						{
							...
							{
								value.SetHeapPtr(arr_ptr, (byte)RtHeapTypeKind.ARRAY);
								break;
							}
							else if (arrStore.StoreMode == RtArray.ArrayStoreMode.cache_on_stack)
							{
								...
								value.SetHeapPtr(arr_heap_ptr, (byte)RtHeapTypeKind.ARRAY);
							}
							else
							{
#if DEBUG
					...
					case RtHeapTypeKind.VECTOR:
						{
							...
							if (vec_ptr < Context.CacheVectorPtr + Context.STACK_LENGTH)
							{
								...
								value.SetHeapPtr(vec_ptr, (byte)RtHeapTypeKind.VECTOR);
							}
						}
		...
		private unsafe void StoreReturnSlot(ref NaNBoxing returnSlot,int stackStPos, int returnSlotIndex, int calleelastpos, int scope_ptr, NaNBoxing value, ref ReceiveError error,bool isyieldreturn_or_holderror =false)
		{
			...
			//if (value.ValueType == NaNBoxing.BoxType.HeapPtr)
			{
				...
				if (value.HeapKind == (byte)RtHeapTypeKind.INSTANCE)
				{
						...
						)
					{
						if (((RtInstance)obj).IsRefVectorOrFromContainerOrStruct(this, (ASInstance)obj.Type))
						{
							...
							returnSlot.SetHeapPtr(clonedptr, (byte)RtHeapTypeKind.INSTANCE);
						}
						else
						{
							RtInstance _temp;
							int t = RtInstance.FindAndUpdateHeapInstancePtr(value.HeapPtr, this, out _temp);
							returnSlot.SetHeapPtr(t, (byte)RtHeapTypeKind.INSTANCE);
						}
					}
					else
					{
						...
						else
						{
							...
							returnSlot.SetHeapPtr(dstptr, (byte)RtHeapTypeKind.INSTANCE);
						}
					}
				}
				else if (value.HeapKind == (byte)RtHeapTypeKind.ARRAY)
				{
					...
					if (arr.StoreMode == RtArray.ArrayStoreMode.cache_on_stack)
					{
						...
						else
						{
							...
							returnSlot.SetHeapPtr(value.HeapPtr, (byte)RtHeapTypeKind.ARRAY);
					...
					else if (arr.StoreMode == RtArray.ArrayStoreMode.cache)
					{
						...
						else
						{
							...
							returnSlot.SetHeapPtr(dstArrayPtr, (byte)RtHeapTypeKind.ARRAY);
						}
				...
				else if (value.HeapKind == (byte)RtHeapTypeKind.VECTOR)
				{
					...
					{
						returnSlot.SetHeapPtr(vec_ptr, (byte)RtHeapTypeKind.VECTOR);
					}
					else
					{
						...
						returnSlot.SetHeapPtr(dstVecPtr, (byte)RtHeapTypeKind.VECTOR);
				...
				else if (value.HeapKind == (byte)RtHeapTypeKind.CLOSURE)
				{
						...
						)
					{
						...
						returnSlot.SetHeapPtr(t, (byte)RtHeapTypeKind.CLOSURE);
					}
					else
					{
						var obj = Context.GC.Heap[value.HeapPtr];
						...
						value.SetHeapPtr(dstClosurePtr, (byte)RtHeapTypeKind.CLOSURE);
						returnSlot = value;
					}
				}
				else
		...
		private unsafe int prepare_savemethodscope_updateref(RtMethodScope heap, int ptr, ref ScopeHeapLocater heapLocater, ASContainer type, int* m_scope, int* method_scopes)
		{
			int* __test = m_scope;
				...
				for (int i = 0; i < scope.SlotCount; ++i)
				{
					if (!(scope == heap && i == heapLocater.MemberIndex))
					{
						...
						if (v.ValueType == NaNBoxing.BoxType.HeapPtr)
						{
							...
							if (v.HeapPtr == ptr || inmember.Kind == RtHeapTypeKind.INSTANCE)
							{
								RtInstance _temp;
								if (v.HeapPtr == ptr || RtInstance.FindAndUpdateHeapInstancePtr(v.HeapPtr, this, out _temp) == ptr)
								{
									if (copyed_ptr == 0)
									{
										//复制一份。
										...
										//更新引用
										v.SetHeapPtr(copyed_ptr, (byte)RtHeapTypeKind.INSTANCE);
										scope.SetSlot(v, (ushort)i);
									}
									else
									{
							...
							else if (inmember.Kind == RtHeapTypeKind.CLOSURE)
							{
								var closure = inmember;
								//lbl_flag:
									...
									if (_this.Kind == RtHeapTypeKind.INSTANCE)
									{
										...
										if (This.HeapPtr == ptr || RtInstance.FindAndUpdateHeapInstancePtr(This.HeapPtr, this, out _temp) == ptr)
										{
											if (copyed_ptr == 0)
											{
												...
												oldPayload.HEAPINSTANCE_PTR = copyed_ptr;
												This.SetHeapPtr(copyed_ptr, (byte)RtHeapTypeKind.INSTANCE);
												if (needupdateScopePtr)
												{
		...
		[MethodImpl(MethodImplOptions.AggressiveOptimization)]
		private unsafe NaNBoxing prepare_savemethodscope_beforeSave(RtMethodScope heap, NaNBoxing old, ScopeHeapLocater heapLocater, int* m_scope, int* method_scopes)
		{
			int* __test = m_scope;
			int max = int.MinValue;
			int min = int.MaxValue;
			...
			if (old.HeapKind == (byte)RtHeapTypeKind.INSTANCE)
			{
				...
				if (ptr == heap.StackPos + heapLocater.MemberIndex + Context.CacheInstancePtr)
				{
					...
					if (((RtInstance)oldObj).methodscopeslot_ref_state == 2) //只有状态是2的情况才可能会被引用
					{
						...
						NaNBoxing r = default;
						r.SetHeapPtr( prepare_savemethodscope_updateref(heap, ptr, ref heapLocater, oldObj.Type, m_scope, method_scopes), (byte)RtHeapTypeKind.INSTANCE);
						return r;
					}
					else
					{
			...
			else if (old.HeapKind == (byte)RtHeapTypeKind.CLOSURE)
			{
				...
				if (ptr == heap.StackPos + heapLocater.MemberIndex + Context.M_ClosurePtr)
				{
					...
					{
						NaNBoxing r = default;
						r.SetHeapPtr(ptr, (byte)RtHeapTypeKind.CLOSURE);
						return r;
						//return ptr;
					}
					else
					{
						...
						if (This.ValueType == NaNBoxing.BoxType.HeapPtr)
						{
							var this_ptr = prepare_savemethodscope_beforeSave(heap, This, heapLocater,m_scope,method_scopes); //更新原this,然后下面才能正确更新.
								...
								if (!(scope == heap && i == heapLocater.MemberIndex))
								{
									...
									if (v.ValueType == NaNBoxing.BoxType.HeapPtr && v.HeapPtr == ptr)
									{
										...
										if (copyed_ptr == 0)
										{
											...
											v.SetHeapPtr(copyed_ptr, (byte)RtHeapTypeKind.CLOSURE);
											scope.SetSlot(v, (ushort)i);
										}
										else
										{
						...
						//return copyed_ptr;
						NaNBoxing r = default;
						r.SetHeapPtr(copyed_ptr, (byte)RtHeapTypeKind.CLOSURE);
						return r;
					}
			...
			else if (old.HeapKind == (byte)RtHeapTypeKind.ARRAY)
			{
				if (ptr == heap.StackPos + heapLocater.MemberIndex + Context.CacheArrayPtr)
				{
					...
					if (((RtArray)oldObj).methodscopeslot_ref_state != 2)
					{
						//return ptr;
						NaNBoxing r = default;
						r.SetHeapPtr(ptr, (byte)RtHeapTypeKind.ARRAY);
						return r;
					}
					else
					{
						...
						for (int k = min; k <= max; k++)
						{
							...
							for (int i = 0; i < scope.SlotCount; ++i)
							{
								if (!(scope == heap && i == heapLocater.MemberIndex))
								{
									...
									{
										var inmember = Context.GC.Heap[v.HeapPtr];
										if (inmember.Kind == RtHeapTypeKind.ARRAY)
										{
											RtArray _temp;
											if (v.HeapPtr == ptr || RtArray.FindAndUpdateHeapInstancePtr(v.HeapPtr, this, out _temp) == ptr)
											{
												if (copyed_ptr == 0)
												{
													copyed_ptr = scope.StackPos + i + Context.CacheArrayPtr;
													...
													v.SetHeapPtr(copyed_ptr, (byte)RtHeapTypeKind.ARRAY);
													scope.SetSlot(v, (ushort)i);
						...
						//return copyed_ptr;
						NaNBoxing r = default;
						r.SetHeapPtr(copyed_ptr, (byte)RtHeapTypeKind.ARRAY);
						return r;
					}
				}
				else
			...
			else if (old.HeapKind == (byte)RtHeapTypeKind.VECTOR)
			{
				if (ptr == heap.StackPos + heapLocater.MemberIndex + Context.CacheVectorPtr)
				{
					...
					{
						NaNBoxing r = default;
						r.SetHeapPtr(ptr, (byte)RtHeapTypeKind.VECTOR);
						return r;
						//return ptr;
					}
					else
						...
						for (int k = min; k <= max; k++)
						{
							...
							for (int i = 0; i < scope.SlotCount; ++i)
							{
								if (!(scope == heap && i == heapLocater.MemberIndex))
								{
									...
									{
										var inmember = Context.GC.Heap[v.HeapPtr];
										if (inmember.Kind == RtHeapTypeKind.VECTOR)
										{
											RtVector _temp;
											if (v.HeapPtr == ptr || RtVector.FindAndUpdateHeapInstancePtr(v.HeapPtr, this, out _temp) == ptr)
											{
												if (copyed_ptr == 0)
												{
													...
													v.SetHeapPtr(copyed_ptr, (byte)RtHeapTypeKind.VECTOR);
													scope.SetSlot(v, (ushort)i);
		...
		[MethodImpl(MethodImplOptions.AggressiveOptimization)]
		private void prepare_savemethodscope_saveinstacne(RtMethodScope heap, ref NaNBoxing saveSlot, RtHeapBase src, int srcPtr, ref ScopeHeapLocater heapLocater,bool is_pass_this)
		{
				...
				)
			{
				...
				saveSlot.SetHeapPtr(clonedptr, (byte)RtHeapTypeKind.INSTANCE);
			...
			else
			{
				...
				if (!(src_ptr < Context.CacheInstancePtr + Context.STACK_LENGTH))
				{
					saveSlot.SetHeapPtr(src_ptr, (byte)RtHeapTypeKind.INSTANCE);
				}
				...
				else
				{
					...
					saveSlot.SetHeapPtr(dstptr, (byte)RtHeapTypeKind.INSTANCE);
		...
		/// </summary>
		[MethodImpl(MethodImplOptions.AggressiveOptimization)]
		private unsafe void PrepareSaveMethodScope(RtMethodScope heap, ref ScopeHeapLocater heapLocater, ref NaNBoxing value, int* m_scope, int* method_scopes, ref ReceiveError error , bool is_pass_this = false)
		{
			if (heap.IsStackSlot)
			{
...
//#endif
				...
				if (value.ValueType == NaNBoxing.BoxType.HeapPtr)
				{
					...
					else if (value.HeapKind == (byte)RtHeapTypeKind.ARRAY)
					{
						...
						else if (array.StoreMode == RtArray.ArrayStoreMode.cache)
						{
							...
							else
							{
								...
								value.SetHeapPtr(dstptr, (byte)RtHeapTypeKind.ARRAY);
					...
					else if (value.HeapKind == (byte)RtHeapTypeKind.VECTOR)
					{
						...
						{
							//pass
							value.SetHeapPtr(vector_ptr , (byte)RtHeapTypeKind.VECTOR);
						}
						...
						else
						{
							...
							value.SetHeapPtr(dstptr, (byte)RtHeapTypeKind.VECTOR);
					...
					else if (value.HeapKind == (byte)RtHeapTypeKind.CLOSURE)
					{
						...
						{
							//它已经在堆里了。
							value.SetHeapPtr(final_ptr, (byte)RtHeapTypeKind.CLOSURE);
						}
			...
			if (dst.ValueType == NaNBoxing.BoxType.HeapPtr && src.ValueType == NaNBoxing.BoxType.HeapPtr && dst.HeapKind == src.HeapKind && dst.HeapKind == (byte)RtHeapTypeKind.INSTANCE )
			{
				...
				//if (oldv.Kind == newv.Kind && oldv.Kind == RtHeapTypeKind.INSTANCE)
				{
					if (((ASInstance)oldv.Type).Flags.HasFlag(ClassFlags.Struct))
					{
						...
						src.SetHeapPtr(dst.HeapPtr, (byte)RtHeapTypeKind.INSTANCE);
						return true;
					}
				}
