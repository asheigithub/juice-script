...
#endif
	{
		...
		private void GetSaveValue_Instance( ref NaNBoxing value, ref ReceiveError error)
		{
			...
			if (src_ptr < Context.CacheInstancePtr + Context.STACK_LENGTH)
			{
				...
				src_obj.LinkToHeap( (RtInstance)heapObj ,ptr);
		...
		private void GetSaveValue_InstanceType(ref NaNBoxing value, ref ReceiveError error)
		{
			...
			//RtHeapBase instance = HeapShotCut[value.HeapPtr];
			if ( value.IsStruct() )//((ASInstance)instance.Type).Flags.HasFlag(ClassFlags.Struct))
			{
				RtHeapBase instance = HeapShortCut[value.HeapPtr];
				//结构体，必要复制一份。
				RtHeapBase heapObj;
		...
		internal NaNBoxing GetSaveValue(NaNBoxing value, ref ReceiveError error)
		{
			if (value.ValueType == NaNBoxing.BoxType.HeapPtr)
			{
				...
				{ 
					RtHeapBase o = HeapShortCut[value.HeapPtr];
		...
		private unsafe void StoreReturnSlot(ref NaNBoxing returnSlot,int stackStPos, int returnSlotIndex, int calleelastpos, int scope_ptr,
			RtMethodScope mscope ,NaNBoxing value, ref ReceiveError error , bool has_finally)
		{
#if DEBUG
			if (value.ValueType != NaNBoxing.BoxType.HeapPtr)
			{
			...
			//if (value.ValueType == NaNBoxing.BoxType.HeapPtr)
			{
				...
				if (value.HeapKind == (byte)RtHeapTypeKind.INSTANCE)
				{
					...
					if (value.IsStruct())
					{
						var obj = HeapShortCut[value.HeapPtr];
						//Clone结构体
					...
					else
					{
						...
						else
						{
							...
							if (!(t < Context.CacheInstancePtr + Context.STACK_LENGTH))
							{
							...
							{
								if (obj.nextframe_ref_state.scope_ptr > 0 &&
										obj.nextframe_ref_state.scope_ptr < scope_ptr &&
										((RtMethodScope)HeapShortCut[obj.nextframe_ref_state.scope_ptr]).version
												== mscope.version
										)
								{
									//那个引用帧还在
								...
								else
								{
									//引用已失效
									obj.nextframe_ref_state.scope_ptr = scope_ptr;
									obj.nextframe_ref_state.version = mscope.version;
								}
								returnSlot.SetHeapPtr(t, (byte)RtHeapTypeKind.INSTANCE, (byte)HeapKindFlag.NONE);
							}
							else
							{
								...
								var dstObj = HeapShortCut[dstptr];
								...
								if (has_finally && obj.methodscopeslot_ref_state != 0)
								{
									//标记returnSlot被引用
									...
									var scope_slots = mscope.__get_slots_internal;
									for (int i = 0; i < mscope.SlotCount; i++)
									{
										ref NaNBoxing sv = ref scope_slots[i];
										if (sv.Raw == value.Raw)
										{
											sv = returnSlot;
											if (obj.methodscopeslot_ref_state == 1)
												break;
										}
									}
				...
				else if (value.HeapKind == (byte)RtHeapTypeKind.ARRAY)
				{
					...
					else
					{
						...
						{							
							var method = ((ASMethodBody)HeapShortCut[scope_ptr].Type).Method;
							int callee_slot_idx = stackStPos - method.Body._link_codescope.Members.Count - 2;

							if (arr.stack_store_startindex + arr.store_memory.Length + 1 < callee_slot_idx)
							{
								//传入的
						...
						else if (arr.StoreMode == RtArray.ArrayStoreMode.cache)
						{
							if (arr_ptr < Context.CacheArrayPtr + calleelastpos) //传入
							{
								...
								if (arr.nextframe_ref_state.scope_ptr > 0 &&
									arr.nextframe_ref_state.scope_ptr < scope_ptr &&
									((RtMethodScope)HeapShortCut[arr.nextframe_ref_state.scope_ptr]).version
											==  mscope.version
									)
								{
									//那个引用帧还在
								...
								else
								{
									//引用已失效
									arr.nextframe_ref_state.scope_ptr = scope_ptr;
									arr.nextframe_ref_state.version = mscope.version;
								}
							...
							else
							{
								...
								var dst = (RtArray)HeapShortCut[dstArrayPtr];

								HeapShortCut[dstArrayPtr].Type = Context.ARRAY.Instance;
								dst.CopyCacheFrom(arr, this, Context.cache_array_memory[returnSlotIndex], Context.cache_array_structindex[returnSlotIndex]);
								...
								if (has_finally && arr.methodscopeslot_ref_state != 0)
								{
									//标记returnSlot被引用
									
									dst.nextframe_ref_state.scope_ptr = scope_ptr;
									dst.nextframe_ref_state.version = mscope.version;
									...
									for (int i = 0; i < mscope.SlotCount; i++)
									{
										...
										{
											sv = returnSlot;
											if (arr.methodscopeslot_ref_state == 1)
												break;
										}
									}
				...
				else if (value.HeapKind == (byte)RtHeapTypeKind.VECTOR)
				{
					...
					else
					{
						...
						else if (vec_ptr < Context.CacheVectorPtr + calleelastpos
							 // 传入
							)
						{
							if (vec.nextframe_ref_state.scope_ptr > 0 &&
									vec.nextframe_ref_state.scope_ptr < scope_ptr &&
									((RtMethodScope)HeapShortCut[vec.nextframe_ref_state.scope_ptr]).version
											== mscope.version
									)
							{
								//那个引用帧还在
							...
							else
							{
								//引用已失效
								vec.nextframe_ref_state.scope_ptr = scope_ptr;
								vec.nextframe_ref_state.version = mscope.version;
							}
						...
						else
						{
							...
							var dstObj = HeapShortCut[dstVecPtr];
							dstObj.Type = HeapShortCut[vec_ptr].Type;
							...
							if (has_finally && vec.methodscopeslot_ref_state != 0)
							{
								//标记returnSlot被引用
								...
								for (int i = 0; i < mscope.SlotCount; i++)
								{
									...
									{
										sv = returnSlot;
										if (vec.methodscopeslot_ref_state == 1)
											break;
									}
								}
				...
				else if (value.HeapKind == (byte)RtHeapTypeKind.CLOSURE)
				{
					...
					else
					{
						...
						if (!(t < Context.M_ClosurePtr + Context.STACK_LENGTH) //堆里
							)
						{
							returnSlot.SetHeapPtr(t, (byte)RtHeapTypeKind.CLOSURE, (byte)HeapKindFlag.NONE);
						}
						else if ((t < Context.M_ClosurePtr + calleelastpos) //传入
							)
						{
							if (obj.nextframe_ref_state.scope_ptr > 0 &&
									obj.nextframe_ref_state.scope_ptr < scope_ptr &&
							...
							if (obj.This.ValueType == NaNBoxing.BoxType.HeapPtr) //更新thisptr的引用
							{
								bool needupdatescopePtr = obj.ScopePtr == obj.This.HeapPtr;
								StoreReturnSlot(ref obj.This, stackStPos, returnSlotIndex, calleelastpos, scope_ptr, mscope, obj.This, ref error, has_finally);

								if (needupdatescopePtr)
								{
									obj.ScopePtr = obj.This.HeapPtr;
								}
						...
						else
						{
							...
							var dstClosure = (RtClosure)HeapShortCut[dstClosurePtr];

							HeapShortCut[dstClosurePtr].Type = obj.Type;

							dstClosure.CopyDataFrom(srcClosure, this);
							dstClosure.methodscopeslot_ref_state = 0;
							dstClosure.nextframe_ref_state = default;

							int origin_srcHeapPtr = srcClosure.HEAPINSTANCE_PTR;

							//srcClosure.HEAPINSTANCE_PTR = dstClosurePtr;
							//srcClosure.LinkToHeap(dstClosure, dstClosurePtr);
							...
							if (dstClosure.This.ValueType == NaNBoxing.BoxType.HeapPtr)
							{
								...
								//var _this = HeapShotCut[dstClosure.This.HeapPtr];
								var _thisKind = (RtHeapTypeKind)dstClosure.This.HeapKind;
								if (_thisKind == RtHeapTypeKind.INSTANCE)
								{
									StoreReturnSlot(ref dstClosure.This, stackStPos, returnSlotIndex, calleelastpos, scope_ptr, mscope, dstClosure.This, ref error,has_finally);
									if (needupdatescopePtr)
									{
							...
							if (srcClosure.HEAPINSTANCE_PTR != origin_srcHeapPtr) //在methodscope加载时，升级到堆了。
							{ 
								dstClosurePtr = srcClosure.HEAPINSTANCE_PTR;
							}

							
							returnSlot.SetHeapPtr(dstClosurePtr, (byte)RtHeapTypeKind.CLOSURE, (byte)HeapKindFlag.NONE);

							if (has_finally && obj.methodscopeslot_ref_state != 0)
							{
								//标记returnSlot被引用

								dstClosure.nextframe_ref_state.scope_ptr = scope_ptr;
								dstClosure.nextframe_ref_state.version = mscope.version;
		...
		private unsafe int prepare_savemethodscope_updateref(RtMethodScope heap, int ptr, ref ScopeHeapLocater heapLocater, int min, int max)
		{
			...
			for (int k = min; k <= max; k++)
			{
				...
				RtMethodScope scope = (RtMethodScope)HeapShortCut[k];
				...
				for (int i = ((k < max || scope.returnSlot < 0) ? 0 : -1); i < scope.SlotCount; ++i)
				{
					...
					{
						NaNBoxing v;
						int store_instacne_ptr;

						if (i == -1) //检查returnSlot槽
						{
							v = Context.StackSlots[scope.returnSlot];
							store_instacne_ptr = scope.returnSlot + Context.CacheInstancePtr;
						}
						else
						{
							v = scope_span[i];
							store_instacne_ptr = scope.StackPos + i + Context.CacheInstancePtr;
						}
						...
						if (v.ValueType == NaNBoxing.BoxType.HeapPtr)
						{
								...
								)
							{
								...
								{
									//复制一份。
									copyed_ptr = store_instacne_ptr; //scope.StackPos + i + Context.CacheInstancePtr;
									var dstObj = (RtInstance)HeapShortCut[copyed_ptr];

									Debug.Assert(copyed_ptr != ptr);
									...
									{
										//更新returnslot槽
										Context.StackSlots[scope.returnSlot] = v;
									}
							...
							{
								var closure = HeapShortCut[v.HeapPtr];
								//lbl_flag:
								...
								if (This.ValueType == NaNBoxing.BoxType.HeapPtr && This.HeapKind == (byte)RtHeapTypeKind.INSTANCE)
								{
									...
									//var _this = HeapShotCut[This.HeapPtr];
									Debug.Assert(HeapShortCut[This.HeapPtr].Kind == RtHeapTypeKind.INSTANCE);
		...
		private  NaNBoxing prepare_savemethodscope_beforeSave(RtMethodScope stackframe, NaNBoxing old, ScopeHeapLocater heapLocater,  int scope_ptr)
		{
			...
			if (old.HeapKind == (byte)RtHeapTypeKind.INSTANCE)
			{
				...
				{
					var oldObj = (RtInstance)HeapShortCut[ptr];
					
					int ref_nextframe = 0;
					if (stackframe.mScopePtr < scope_ptr && oldObj.nextframe_ref_state.scope_ptr > 0
						&& ((RtMethodScope)HeapShortCut[oldObj.nextframe_ref_state.scope_ptr]).version == oldObj.nextframe_ref_state.version)
					{
						ref_nextframe = 1;
					}
					else
					{
						oldObj.nextframe_ref_state = default;
					}
					...
					if (oldObj.methodscopeslot_ref_state + ref_nextframe >= 2) //只有状态是2的情况才可能会被引用
					{
						...
						/*			这是最差情况，确实会导致没有引用了但是还需要查找一次	 但为了避免在赋值时还需要取旧对象，权衡之下先保留，因为引用计数会导致必须到堆里查找旧对象		 
						(function k1()
						{
							var i = {};
							var j = i;
			...
			else if (old.HeapKind == (byte)RtHeapTypeKind.CLOSURE)
			{
				...
				{
					var oldObj = (RtClosure)HeapShortCut[ptr];
						...
						for (int k = min; k <= max; k++)
						{
							...
							for (int i = ((k < max || scope.returnSlot < 0) ? 0 : -1); i < scope.SlotCount; ++i)
							{
								if (!(scope == stackframe && i == heapLocater.MemberIndex))
								{
									...
									{
										v = Context.StackSlots[scope.returnSlot];
										store_closure_ptr = scope.returnSlot + Context.M_ClosurePtr;
									}
									else
									{
										v = scope_span[i];
										store_closure_ptr = scope.StackPos + i + Context.M_ClosurePtr;
									}
										...
										)
									{
										...
										if (copyed_ptr == 0)
										{
											copyed_ptr = store_closure_ptr;
											var dstClosure = (RtClosure)HeapShortCut[copyed_ptr];
										...
										else
										{
											if (toupate_ref != null && k == stackframe.mScopePtr)
											{
												...
												{
													var newthis = (RtInstance)HeapShortCut[toupate_ref.This.HeapPtr];
			...
			else if (old.HeapKind == (byte)RtHeapTypeKind.ARRAY)
			{
				...
				{
					var oldObj = (RtArray)HeapShortCut[ptr];
#if DEBUG
						...
						for (int k = min; k <= max; k++)
						{
							...
							for (int i = ( ( k<max || scope.returnSlot<0) ? 0 : -1) ; i < scope.SlotCount; ++i)
							{
								...
								if (!(scope == stackframe && i == heapLocater.MemberIndex))
								{
									...
									{
										v = Context.StackSlots[scope.returnSlot];
										store_arr_ptr = scope.returnSlot + Context.CacheArrayPtr;
									}
									else
									{
										v = scope_span[i];
										store_arr_ptr = scope.StackPos + i + Context.CacheArrayPtr;
									}


									if (v.ValueType == NaNBoxing.BoxType.HeapPtr && v.HeapKind == (byte)RtHeapTypeKind.ARRAY)
									{
										//var inmember = HeapShotCut[v.HeapPtr];
										Debug.Assert(HeapShortCut[v.HeapPtr].Kind == RtHeapTypeKind.ARRAY);


										if (v.HeapPtr == ptr 
											//|| RtArray.FindAndUpdateHeapInstancePtr(v.HeapPtr, this, out RtArray _temp) == ptr
											)
										{
											if (copyed_ptr == 0)
											{
												copyed_ptr = store_arr_ptr;

												Debug.Assert(copyed_ptr != ptr);

												var dst = (RtArray)HeapShortCut[copyed_ptr];
												//if (copyed_ptr != ptr)
												{
													...
													((RtArray)dst).CopyCacheFrom(oldPayload, this, Context.cache_array_memory[scope.StackPos + i], Context.cache_array_structindex[scope.StackPos + i]);
			...
			else if (old.HeapKind == (byte)RtHeapTypeKind.VECTOR)
			{
				...
				{
					var oldObj = (RtVector)HeapShortCut[ptr];
						...
						for (int k = min; k <= max; k++)
						{
							...
							for (int i = ((k < max || scope.returnSlot < 0) ? 0 : -1); i < scope.SlotCount; ++i)
							{
								if (!(scope == stackframe && i == heapLocater.MemberIndex))
								{
									...
									{
										v = Context.StackSlots[scope.returnSlot];
										store_vec_ptr = scope.returnSlot + Context.CacheVectorPtr;
									}
									else
									{
										v = scope_span[i];
										store_vec_ptr = scope.StackPos + i + Context.CacheVectorPtr;
									}
									...
									{
										//var inmember = HeapShotCut[v.HeapPtr];
										Debug.Assert(HeapShortCut[v.HeapPtr].Kind == RtHeapTypeKind.VECTOR);
											...
											)
										{
											if (copyed_ptr == 0)
											{
												copyed_ptr = store_vec_ptr;

												var dst = (RtVector)HeapShortCut[copyed_ptr];
		...
		private void prepare_savemethodscope_saveinstance(RtMethodScope heap, ref NaNBoxing saveSlot,   ScopeHeapLocater heapLocater, int scope_ptr ,bool is_pass_this)
		{
				...
				)
			{
				...
				{
					var src = HeapShortCut[saveSlot.HeapPtr];
					//Clone结构体
				...
				{
					Debug.Assert(((RtInstance)HeapShortCut[saveSlot.HeapPtr]).IsRefVectorOrFromContainerOrRefStruct(this, (ASInstance)HeapShortCut[saveSlot.HeapPtr].Type)
						== ((((HeapKindFlag)saveSlot.HeapFlag & HeapKindFlag.FLAG_REFSTRUCT) == HeapKindFlag.FLAG_REFSTRUCT))
						);
				}
#endif
			...
			else
			{
				...
				byte heapflag = (byte)(((ASInstance)srcPayload.Type).Flags.HasFlag(ClassFlags.Struct) ? HeapKindFlag.FLAG_STRUCT : HeapKindFlag.NONE);

				//如果在堆里，直接存
				if (!(src_ptr < Context.CacheInstancePtr + Context.STACK_LENGTH))
				{
					saveSlot.SetHeapPtr(src_ptr, (byte)RtHeapTypeKind.INSTANCE, heapflag);
				}
				else if (src_ptr < heap.StackPos + Context.CacheInstancePtr)
				{
					...
					if (srcPayload.nextframe_ref_state.scope_ptr > 0 &&
						...
						((RtMethodScope)HeapShortCut[srcPayload.nextframe_ref_state.scope_ptr]).version
				...
				else
				{
					...
					var dstObj = (RtInstance)HeapShortCut[dstptr];
					...
					((RtInstance)dstObj).nextframe_ref_state = default;
					...
					if (srcPayload.methodscopeslot_ref_state != 0 ||
							(srcPayload.nextframe_ref_state.scope_ptr > 0 && srcPayload.nextframe_ref_state.scope_ptr <= scope_ptr &&
							srcPayload.nextframe_ref_state.version == ((RtMethodScope)HeapShortCut[scope_ptr]).version)
							)
					{
						//查找引用src_ptr的槽，改为指向dst
						for (int i = heap.mScopePtr + 1; i <= scope_ptr; i++)
						{
							var scope = (RtMethodScope)HeapShortCut[i];
							//if (src_ptr < (scope.returnSlot < 0 ? scope.StackPos : scope.returnSlot) + Context.CacheInstancePtr)
							//	continue;

							var scope_slots = scope.__get_slots_internal;
							for (int j = 0; j < scope_slots.Length; j++)
							{
								
								ref NaNBoxing sv = ref scope_slots[j];
								if (sv.Raw == saveSlot.Raw)
								{
									if (dstObj.nextframe_ref_state.scope_ptr == 0)
									{
										dstObj.nextframe_ref_state.scope_ptr = i;
										dstObj.nextframe_ref_state.version = scope.version;
									}

									sv.SetHeapPtr(dstptr, (byte)RtHeapTypeKind.INSTANCE, heapflag);
								}
								else if (sv.ValueType == NaNBoxing.BoxType.HeapPtr && sv.HeapKind == (byte)RtHeapTypeKind.CLOSURE) //搬移时还需要更新closure的引用
								{
									RtClosure closure = (RtClosure)(HeapShortCut[sv.HeapPtr]);
									if (closure.This.Raw == saveSlot.Raw)
									{
										...
										closure.This.SetHeapPtr(dstptr, (byte)RtHeapTypeKind.INSTANCE, heapflag);

										if (closure.ScopePtr == saveSlot.HeapPtr)
										{
											closure.ScopePtr = dstptr;
										}

									}
								}
							}
							if (i == scope_ptr && scope.returnSlot >= 0)
							{								
								ref NaNBoxing sv = ref Context.StackSlots[scope.returnSlot];
								if (sv.Raw == saveSlot.Raw)
								{
									if (dstObj.nextframe_ref_state.scope_ptr == 0)
									{
								...
								else if (sv.ValueType == NaNBoxing.BoxType.HeapPtr && sv.HeapKind == (byte)RtHeapTypeKind.CLOSURE)
								{
									RtClosure closure = (RtClosure)(HeapShortCut[sv.HeapPtr]);
									if (closure.This.Raw == saveSlot.Raw)
									{
					...
					saveSlot.SetHeapPtr(dstptr, (byte)RtHeapTypeKind.INSTANCE, heapflag);
			...
			NaNBoxing old,int min, int scope_ptr, ref ReceiveError error ,bool is_pass_this)
		{
			
			{
				...
				if (value.HeapKind == (byte)RtHeapTypeKind.INSTANCE)
				{
					...
					//var obj = HeapShotCut[value.HeapPtr];
						...
						else if (array.StoreMode == RtArray.ArrayStoreMode.cache)
						{
							...
							if (array_ptr < heap.StackPos + Context.CacheArrayPtr)
							{
								...
								if (array.nextframe_ref_state.scope_ptr > 0 &&
									...
									((RtMethodScope)HeapShortCut[array.nextframe_ref_state.scope_ptr]).version == array.nextframe_ref_state.version
									)
								{
							...
							else
							{
								...
								var dstObj = (RtArray)HeapShortCut[dstptr];
								...
								if (array.methodscopeslot_ref_state != 0 ||
									(array.nextframe_ref_state.scope_ptr > 0 && array.nextframe_ref_state.scope_ptr <= scope_ptr &&
									array.nextframe_ref_state.version == ((RtMethodScope)HeapShortCut[scope_ptr]).version)
									)
								{
									//查找引用array_ptr的槽，改为指向dst
									for (int i = heap.mScopePtr + 1; i <= scope_ptr; i++)
									{
										var scope = (RtMethodScope)HeapShortCut[i];
										//if (array_ptr < (scope.returnSlot < 0 ? scope.StackPos : scope.returnSlot) + Context.CacheArrayPtr)
										//	continue;
										...
										for (int j = 0; j < scope_slots.Length; j++)
										{
											...
											if (sv.Raw == value.Raw)
											{
												...
												sv.SetHeapPtr(dstptr, (byte)RtHeapTypeKind.ARRAY, (byte)HeapKindFlag.NONE);
											}
										}
										if (i == scope_ptr && scope.returnSlot >= 0)
										{
					...
					if (value.HeapPtr < Context.CacheVectorPtr + Context.STACK_LENGTH)
					{
						...
						else if (vector_ptr < heap.StackPos + Context.CacheVectorPtr)
						{
							...
							if (vector.nextframe_ref_state.scope_ptr > 0 &&
								...
								((RtMethodScope)HeapShortCut[vector.nextframe_ref_state.scope_ptr]).version == vector.nextframe_ref_state.version
								)
							{
						...
						else
						{
							...
							var dstObj = (RtVector)HeapShortCut[dstptr];
							dstObj.Type = obj.Type;
							...
							if (vector.methodscopeslot_ref_state != 0 ||
								(vector.nextframe_ref_state.scope_ptr > 0 && vector.nextframe_ref_state.scope_ptr <= scope_ptr &&
								vector.nextframe_ref_state.version == ((RtMethodScope)HeapShortCut[scope_ptr]).version)
								)
							{
								//查找引用vector_ptr的槽，改为指向dst
								for (int i = heap.mScopePtr + 1; i <= scope_ptr; i++)
								{
									var scope = (RtMethodScope)HeapShortCut[i];
									//if (vector_ptr < (scope.returnSlot < 0 ? scope.StackPos : scope.returnSlot) + Context.CacheVectorPtr)
									//	continue;
									...
									for (int j = 0; j < scope_slots.Length; j++)
									{
										...
										if (sv.Raw == value.Raw)
										{
											...
											sv.SetHeapPtr(dstptr, (byte)RtHeapTypeKind.VECTOR, (byte)HeapKindFlag.NONE);
										}
									}
									if (i == scope_ptr && scope.returnSlot >= 0)
									{
					...
					if (value.HeapPtr < Context.M_ClosurePtr + Context.STACK_LENGTH)
					{
						...
						else if (final_ptr < heap.StackPos + Context.M_ClosurePtr)
						{
							...
							if (srcClosure.nextframe_ref_state.scope_ptr > 0
								...
								&& ((RtMethodScope)HeapShortCut[srcClosure.nextframe_ref_state.scope_ptr]).version
							...
							{
								var _this = HeapShortCut[srcClosure.This.HeapPtr];
						...
						else
						{
							...
							HeapShortCut[dstClosurePtr].Type = srcClosure.Type;
							...
							if (srcClosure.methodscopeslot_ref_state != 0 ||
								(srcClosure.nextframe_ref_state.scope_ptr > 0 && srcClosure.nextframe_ref_state.scope_ptr <= scope_ptr &&
								srcClosure.nextframe_ref_state.version == ((RtMethodScope)HeapShortCut[scope_ptr]).version)
								)
							{
								//查找引用final_ptr的槽，改为指向dst
								for (int i = heap.mScopePtr + 1; i <= scope_ptr; i++)
								{
									var scope = (RtMethodScope)HeapShortCut[i];
									//if (final_ptr < (scope.returnSlot < 0 ? scope.StackPos : scope.returnSlot) + Context.M_ClosurePtr)
									//	continue;
									...
									for (int j = 0; j < scope_slots.Length; j++)
									{
										...
										if (sv.Raw == value.Raw)
										{
											if (dstClosure.nextframe_ref_state.scope_ptr == 0)
											{
												dstClosure.nextframe_ref_state.scope_ptr = i;
												dstClosure.nextframe_ref_state.version = scope.version;
											}

											sv.SetHeapPtr(final_ptr, (byte)RtHeapTypeKind.CLOSURE, (byte)HeapKindFlag.NONE);
										}
									}
									if (i == scope_ptr && scope.returnSlot >= 0)
									{
		...
		internal bool CopyIfSameTypeStructAndReplaceSrc(NaNBoxing dst,ref NaNBoxing src)
		{
			...
			{
				var oldv = HeapShortCut[dst.HeapPtr];
				var newv = HeapShortCut[src.HeapPtr];
