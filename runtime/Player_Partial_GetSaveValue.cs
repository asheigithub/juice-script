...
namespace juicescript.runtime
{
...
#endif
	{
		...
		/// </summary>
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private unsafe void PrepareSaveMethodScope(RtMethodScope heap,  ScopeHeapLocater heapLocater, ref NaNBoxing value,  int scope_ptr, ref ReceiveError error , bool is_pass_this = false)
		{
			if (heap.IsStackSlot)
			{
				...
				{
					//存储阶段
					prepare_savescope_pass(ref value, heap, heapLocater, old, min, max, scope_ptr, ref error, is_pass_this);
				}
			}
			else
			{
