...
#endif
	{
		...
		private void GetSaveValue_Instance( ref NaNBoxing value, ref ReceiveError error)
		{
			...
			if (src_ptr < Context.CacheInstancePtr + Context.STACK_LENGTH)
			{
				...
				RtHeapBase heapObj;
				int ptr = Context.GC.AllocInstance((ASInstance)target.Type, out heapObj);
				if (ptr == 0)
				{
				...
				((RtInstance)heapObj).CopyFrom(target, (ASInstance)heapObj.Type, this, target.Type._link_codescope.TypeLayout.Size);
		...
		[MethodImpl(MethodImplOptions.AggressiveOptimization)]
		private unsafe int prepare_savemethodscope_updateref(RtMethodScope heap, int ptr, ref ScopeHeapLocater heapLocater, ASContainer type, int min, int max)
		{
		...
		private  NaNBoxing prepare_savemethodscope_beforeSave(RtMethodScope heap, NaNBoxing old, ScopeHeapLocater heapLocater, ref int min, ref int max,int scope_ptr)
		{
			...
			if (old.HeapKind == (byte)RtHeapTypeKind.INSTANCE)
			{
				...
				if (ptr == heap.StackPos + heapLocater.MemberIndex + Context.CacheInstancePtr)
				{
					...
					if (((RtInstance)oldObj).methodscopeslot_ref_state == 2) //只有状态是2的情况才可能会被引用
					{
						...
						NaNBoxing r = default;
						r.SetHeapPtr( 
							prepare_savemethodscope_updateref(heap, ptr, ref heapLocater, oldObj.Type, min, max), (byte)RtHeapTypeKind.INSTANCE,
			...
			else if (old.HeapKind == (byte)RtHeapTypeKind.ARRAY)
			{
				if (ptr == heap.StackPos + heapLocater.MemberIndex + Context.CacheArrayPtr)
				{
					...
					if (oldObj.methodscopeslot_ref_state + (oldObj.thisslot_ref_state !=0?1:0) <2 )
					{
						//return ptr;
						NaNBoxing r = default;
										...
										if (v.HeapPtr == ptr || RtArray.FindAndUpdateHeapInstancePtr(v.HeapPtr, this, out RtArray _temp) == ptr)
										{
											if (copyed_ptr == 0)
											{
												...
												if (copyed_ptr != ptr)
												{
													...
													{
														//说明是this槽。
														((RtArray)dst).thisslot_ref_state = (byte)k;
												...
												if (i != scope.SlotCount - 1 && dst.methodscopeslot_ref_state==1) //若是存入this槽内，那么没有再将this置空，所以放在这里是安全的，无需再更新引用。method结束后，外部也不可能再有指向this的引用了。
												{
		...
		private void prepare_savescope_pass(ref NaNBoxing value, RtMethodScope heap,ScopeHeapLocater heapLocater,
			NaNBoxing old,int min,int max, int scope_ptr,out RtHeapBase save_as_this, ref ReceiveError error ,bool is_pass_this)
		{
			
			{
				...
				else if (value.HeapKind == (byte)RtHeapTypeKind.ARRAY)
				{
					...
					else if (array.StoreMode == RtArray.ArrayStoreMode.cache)
					{
						...
						if (array_ptr < heap.StackPos + heap.SlotCount + Context.CacheArrayPtr)
						{
							...
							if (is_pass_this)
							{
								if (array.thisslot_ref_state == 0) //标记被this引用.
								{
									save_as_this = array;
									array.thisslot_ref_state = (byte)scope_ptr;
								}
								else
								{
									save_as_this = null;
						...
						else
						{
							...
							((RtArray)dstObj).thisslot_ref_state = 0;
				...
				else if (value.HeapKind == (byte)RtHeapTypeKind.VECTOR)
				{
					...
					else if (vector_ptr < heap.StackPos + heap.SlotCount + Context.CacheVectorPtr)
					{
						...
						if (is_pass_this)
						{							
							if (vector.thisslot_ref_state == 0) //标记被this引用.
							{
								save_as_this = vector;
								vector.thisslot_ref_state = (byte)scope_ptr;
							}
							else
							{
								save_as_this = null;
		...
		private unsafe void PrepareSaveMethodScope(RtMethodScope heap,  ScopeHeapLocater heapLocater, ref NaNBoxing value, int* m_scope, int* method_scopes, ref ReceiveError error , bool is_pass_this = false)
		{
			if (heap.IsStackSlot)
			{
				...
				{
					//存储阶段
					prepare_savescope_pass(ref value, heap, heapLocater, old, min, max, *method_scopes,out RtHeapBase saved, ref error, is_pass_this);
				}
			}
			else
			{
